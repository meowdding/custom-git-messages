...
const defaulBranches = (ref) => ["master", "main", "stable", "dev", "development"].some((branch) => ref === `refs/heads/${branch}`);
...
export const Push = async (body: any): Promise<GithubMessage | undefined> => {
  ...
  let allowedBranchFilter = projects[repoName.toLowerCase()]?.push_branches || defaulBranches;

  const defaultBranchRef = `refs/heads/${body.repository.default_branch}`;
  const isAllowedBranch = allowedBranchFilter(ref);
